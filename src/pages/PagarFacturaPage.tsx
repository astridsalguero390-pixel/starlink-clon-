                                ...
                                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                    ...
                                    <div className="bg-card border border-border rounded-2xl p-6 md:p-8">
                                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                                            ...
                                            <div className={`inline-flex px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${factura.estado === 'pagado' ? 'bg-green-500/10 text-green-500 border border-green-500/20' :
                                                    factura.estado === 'verificando' ? 'bg-yellow-500/10 text-yellow-500 border border-yellow-500/20' :
                                                        factura.estado === 'rechazado' ? 'bg-red-500/10 text-red-500 border border-red-500/20' :
                                                            'bg-foreground/5 text-foreground/60 border border-border'
                                                }`}>
                                                {factura.estado === 'pendiente' && '📝 Pendiente'}
                                                {factura.estado === 'verificando' && '🔍 En Revisión'}
                                                {factura.estado === 'pagado' && '✅ Pagada'}
                                    ...
                                    {factura.estado === 'pagado' ? (
                                        <div className="bg-green-500/5 border border-green-500/20 rounded-2xl p-8 md:p-12 text-center">
                                            <div className="bg-green-500/10 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6">
                                                <CheckCircle2 size={40} className="text-green-500" />
                                            </div>
                                            <h3 className="text-2xl font-bold tracking-tight text-foreground mb-3">Factura Pagada</h3>
                                            <p className="text-muted-foreground max-w-md mx-auto">
                                                El pago de esta factura ha sido procesado y confirmado correctamente. Gracias por preferir Starlink.
                                            </p>
                                        </div>
                                    ) : factura.estado === 'verificando' ? (
                                        <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-2xl p-8 md:p-12 text-center">
                                            <div className="bg-yellow-500/10 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6">
                                                <Search size={40} className="text-yellow-500 animate-pulse" />
                                            </div>
                                            <h3 className="text-2xl font-bold tracking-tight text-foreground mb-3">Pago en Revisión</h3>
                                            <p className="text-muted-foreground max-w-md mx-auto">
                                                Hemos recibido su comprobante de pago. Nuestro equipo administrativo lo está validando. Le notificaremos brevemente.
                                            </p>
                                        </div>
                                    ) : (
                                        <>
